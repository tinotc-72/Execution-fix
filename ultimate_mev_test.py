"""
Ultimate Fix - Remove fee_config Account Entirely
Based on error analysis, maybe fee_config is not needed at all!
"""
...
class UltimateMEVBot:
    """
    Ultimate MEV bot - remove fee_config entirely and test different account structures
    """
    ...
    async def test_structure_1(self, mint_address: str, sol_amount: float) -> bool:
        """Test without fee_config at all - 13 accounts"""
        try:
            logger.info("🧪 Testing Structure 1: NO fee_config (13 accounts)")
            ...
            # 13 account structure (NO fee_config)
            account_metas = [
            ...
            return await self._test_structure(account_metas, instruction_data, "Structure 1 (13 accounts)")
            
        except Exception as e:
            logger.error(f"❌ Structure 1 error: {e}")
            return False
    
    async def test_structure_2(self, mint_address: str, sol_amount: float) -> bool:
        """Test with doubled programs - maybe missing some"""
        try:
            logger.info("🧪 Testing Structure 2: Doubled programs")
            ...
            # Maybe we need to repeat some programs
            account_metas = [
                AccountMeta(pubkey=self.GLOBAL_ACCOUNT, is_signer=False, is_writable=False),           # Global
                AccountMeta(pubkey=mint_pubkey, is_signer=False, is_writable=False),                  # Mint
                AccountMeta(pubkey=bonding_curve, is_signer=False, is_writable=True),                 # Bonding curve
                ...
                AccountMeta(pubkey=self.ASSOCIATED_TOKEN_PROGRAM, is_signer=False, is_writable=False), # ATA program
                AccountMeta(pubkey=self.FEE_RECIPIENT, is_signer=False, is_writable=True),            # Fee recipient
                AccountMeta(pubkey=self.PUMP_FUN_PROGRAM, is_signer=False, is_writable=False),        # Pump.fun program again
                AccountMeta(pubkey=self.TOKEN_PROGRAM, is_signer=False, is_writable=False),           # Token program again
            ]
            
            return await self._test_structure(account_metas, instruction_data, "Structure 2 (15 accounts)")
            
        except Exception as e:
            logger.error(f"❌ Structure 2 error: {e}")
            return False
    
    async def test_structure_3(self, mint_address: str, sol_amount: float) -> bool:
        """Test with all possible sysvar accounts"""
        try:
            logger.info("🧪 Testing Structure 3: With all sysvars")
            ...
            account_metas = [
                ...
                AccountMeta(pubkey=self.TOKEN_PROGRAM, is_signer=False, is_writable=False),           # Token program
                AccountMeta(pubkey=self.RENT_SYSVAR, is_signer=False, is_writable=False),             # Rent sysvar
                AccountMeta(pubkey=CLOCK_SYSVAR, is_signer=False, is_writable=False),                 # Clock sysvar
                AccountMeta(pubkey=SLOT_HASHES_SYSVAR, is_signer=False, is_writable=False),           # Slot hashes sysvar
                AccountMeta(pubkey=self.EVENT_AUTHORITY, is_signer=False, is_writable=False),         # Event authority
                AccountMeta(pubkey=self.PUMP_FUN_PROGRAM, is_signer=False, is_writable=False),        # Pump.fun program
                AccountMeta(pubkey=self.ASSOCIATED_TOKEN_PROGRAM, is_signer=False, is_writable=False), # ATA program
                AccountMeta(pubkey=self.FEE_RECIPIENT, is_signer=False, is_writable=True),            # Fee recipient
            ...
            return await self._test_structure(account_metas, instruction_data, "Structure 3 (15 accounts)")
            
        except Exception as e:
            logger.error(f"❌ Structure 3 error: {e}")
            return False
    
    async def _test_structure(self, account_metas, instruction_data, structure_name: str) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if 'result' in result:
                    ...
                    logger.info(f"🎉 SUCCESS with {structure_name}! Signature: {signature}")
                    return True
                else:
                    error = result.get('error', {})
                    error_msg = error.get('message', str(error))
                    logger.warning(f"❌ {structure_name}: {error_msg}")
                    return False
                    
        except Exception as e:
            logger.error(f"❌ {structure_name} test error: {e}")
            return False
    
    async def test_all_structures(self, mint_address: str, sol_amount: float):
        """Test all different account structures"""
        
        logger.info(f"🚀 TESTING ALL ACCOUNT STRUCTURES")
        logger.info(f"   Mint: {mint_address}")
        logger.info(f"   Amount: {sol_amount} SOL")
        logger.info("=" * 60)
        
        structures = [
            self.test_structure_1,
            self.test_structure_2,
            self.test_structure_3
        ]
        
        for i, test_func in enumerate(structures):
            success = await test_func(mint_address, sol_amount)
            
            if success:
                logger.info(f"🏆 WINNER: Structure {i+1}")
                return True
            
            await asyncio.sleep(2)  # Small delay between tests
        
        logger.error("❌ No working structure found!")
        return False

async def run_ultimate_tests():
    """Run ultimate account structure tests"""
    
    env = EnvKeys()
    bot = UltimateMEVBot(env)
    ...
    # Test all structures
    success = await bot.test_all_structures(test_mint, test_amount)
    ...
    if success:
        ...
        print()
        print("❌ Still need to investigate account structure further.")

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    asyncio.run(run_ultimate_tests())
