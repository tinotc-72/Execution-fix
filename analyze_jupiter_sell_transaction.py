...
def analyze_jupiter_sell_transaction():
    """Analyze Jupiter sell transaction to validate bot's Jupiter sell logic"""
    
    print("🔍 ANALYZING JUPITER SELL TRANSACTION")
    print("="*60)
    
    signature = "3ju6vtB1jPMEfLi5WcMLozEvFEd4H8iDwC289LpC2sEpeCxJr3BQEjjATuaYqGPTiN5PBGAiTNp5u4dfw6TR6zQ7"
        ...
        if response.status_code == 200:
            ...
            if 'tokenTransfers' in data:
                ...
                for i, transfer in enumerate(transfers):
                    ...
                    print(f"     From: {from_addr[:8]}...")
                    print(f"     To: {to_addr[:8]}...")
            ...
            if 'tokenTransfers' in data:
                for transfer in data['tokenTransfers']:
                    ...
                    if token_amount > 0:
                        ...
                        token_mints_involved.add(token_mint[:8] + "...")
            ...
            print(f"   📉 Tokens involved: {len(token_mints_involved)} types")
            ...
            if 'result' in rpc_data and rpc_data['result']:
                ...
                if 'transaction' in tx_data and 'message' in tx_data['transaction']:
                    ...
                    for i, instruction in enumerate(instructions):
                        ...
                        # Look for Jupiter program ID
                        if program_id == "JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4":
                            jupiter_patterns.append(f"Instruction {i+1}: Jupiter V6 program")
                        elif program_id == "JUP4Fb2cqiRUcaTHdrPC8h2gNsA2ETXiPDD33WcGuJB":
                            jupiter_patterns.append(f"Instruction {i+1}: Jupiter V4 program")
                            
                    if jupiter_patterns:
                        print(f"   🪐 Jupiter patterns found:")
                        for pattern in jupiter_patterns:
                            print(f"     {pattern}")
                    else:
                        print(f"   🔍 No obvious Jupiter patterns in instructions")
                        
                    print(f"   📋 Unique programs involved: {len(program_ids)}")
    ...
    print(f"\n{'='*60}")
    print("🎯 JUPITER SELL ANALYSIS SUMMARY:")
    print("="*60)
    print("📊 This analysis will help verify your bot's Jupiter sell logic")
    print("🔧 Comparing patterns with your Jupiter copy executor")
    print("💡 Ensuring compatibility with multi-DEX selling strategy")

if __name__ == "__main__":
    analyze_jupiter_sell_transaction()
