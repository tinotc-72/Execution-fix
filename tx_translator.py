...
from config import RPC_URL, WALLET_A_ADDRESS
    ...
    try:
        ...
        for ix in original_msg.instructions:
            ...
            for i in ix.accounts:
                ...
                # Replace Wallet A with your wallet
                new_key = payer if old_key == WALLET_A else old_key

                # Rewrite known PDAs if they depend on Wallet A
                if old_key not in [payer, new_key] and old_key != program_id:
                    new_key = rewrite_pda_if_wallet_a(old_key, program_id, WALLET_A, payer)
                ...
                if "Tokenkeg" in str(program_id) and old_key == WALLET_A:
