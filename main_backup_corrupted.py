...
class CopyTradingBot:
    ...
    async def handle_account_notification(self, result: Dict[str, Any]):
            ...
            # 🚀 ULTRA-AGGRESSIVE: If we have DEX + trade type but no token, extract ANY token
            if dex_detected and trade_type and not token_mint:
                # Look for any token-like string in the logs
                for token in token_matches:
                    if (len(token) >= 40 and  # Even shorter addresses
                        token != target_wallet and
                        not token.startswith('1111111')):  # Avoid system programs
                        token_mint = token
                        logger.info(f"🚀 AGGRESSIVE TOKEN EXTRACTION: Using {token_mint[:8]}...")
                        break
            ...
            elif dex_detected and trade_type:
                logger.info(f"⚡ PARTIAL DETECTION: {trade_type.upper()} on {dex_detected} - no token found")
                logger.info(f"� AGGRESSIVE MODE: Executing with DEX info anyway!")
                
                # 🚀 ULTRA-AGGRESSIVE: Create minimal trade info even without clear token
                # Since you trust the wallet, any DEX activity is potentially worth copying
                trade_info = {
                    'type': trade_type,
                    'token_mint': 'PARTIAL_DETECTION',  # Will be analyzed later
                    'amount': self.config.investment_amount_sol,
                    'timestamp': datetime.now(),
                    'dex': dex_detected,
                    'detection_method': 'aggressive_partial'
                }
                
                # Execute with fallback to full analysis
                logger.info(f"🔄 Falling back to full analysis for token extraction...")
                asyncio.create_task(self.analyze_transaction(signature, target_wallet))
            else:
                logger.debug(f"📝 Non-trade or unclear: {signature[:8]}...")
