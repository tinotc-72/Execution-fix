...
class TradeExecutor:
    def __init__(self, wallet: Keypair, tracker=None):
        ...
        self.setup = AccountSetup(wallet)
    ...
    async def validate_accounts(self, token_mint: Pubkey) -> bool:
        """Validate all required accounts before trading"""
        try:
            # 1. Check ATA exists
            ata_exists = await self.setup.check_and_create_ata(token_mint)
            if not ata_exists:
                print("❌ Failed to validate Associated Token Account")
                return False
                
            # 2. Check user account exists
            user_account_exists = await self.setup.check_and_create_user_account()
            if not user_account_exists:
                print("❌ Failed to validate user account")
                return False
            ...
            print(f"❌ Account validation error: {str(e)}")
            traceback.print_exc()
            return False
    ...
    async def execute_trade(self, token_mint: str, amount: float, is_buy: bool) -> bool:
        """Execute a trade with account validation"""
        try:
            token_pubkey = Pubkey.from_string(token_mint)
            
            # Validate accounts first
            print("\n🔍 Validating accounts...")
            if not await self.validate_accounts(token_pubkey):
                print("❌ Account validation failed, cannot proceed with trade")
                return False
