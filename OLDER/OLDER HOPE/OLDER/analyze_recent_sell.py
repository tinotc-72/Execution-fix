#!/usr/bin/env python3
"""
Analyze recent successful pump.fun sell transactions to understand the correct account structure
"""
...
TARGET_TOKEN = "6nTfw8wkRRqJviT9JZvRyeR2WV682ETZEf7LudQzpump"  # Our test token

# Environment
keys = EnvKeys()
RPC_URL = f"https://mainnet.helius-rpc.com/v0/?api-key={keys.HELIUS_API_KEY}"

async def find_recent_sell_transactions() -> List[Dict[str, Any]]:
    """Find recent successful sell transactions for our target token"""
    logger.info(f"🔍 Searching for recent sell transactions for token: {TARGET_TOKEN}")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i, sig_info in enumerate(signatures):
            ...
            # Check if this is a sell transaction for our token
            if is_sell_transaction_for_token(tx_result, TARGET_TOKEN):
...
def is_sell_transaction_for_token(tx_data: Dict[str, Any], token_mint: str) -> bool:
        ...
        for instruction in instructions:
            if instruction.get("programId") == PUMP_TRADE_PROGRAM:
                ...
                if data.startswith(SELL_DISCRIMINATOR):
                    # Check if token mint is involved
...
def identify_account_type(pubkey: str) -> str:
    ...
    try:
        ...
        # Could add more sophisticated identification here
        return "Unknown"
    except:
...
async def main():
    ...
    logger.info("🚀 Starting analysis of recent sell transactions")
    logger.info("=" * 60)
    
    try:
        # Find recent sell transactions
        sell_txs = await find_recent_sell_transactions()
        
        if not sell_txs:
            logger.warning("❌ No sell transactions found for the target token")
            return
