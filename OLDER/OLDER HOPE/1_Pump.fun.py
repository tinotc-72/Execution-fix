# 1_Pump.fun.py - works for pump.fun - buy - hold - sell in less than 6 seconds
# !/usr/bin/env python3 - this is the bot meant to be buying and selling
"""
Production-Ready Pump.Fun Trading Bot
Complete autonomous trading system with buy, hold, and sell capabilities
Supports both manual trading and copy trading functionality
"""
...
class PumpFunTradingBot:
    """
    Production-ready Pump.Fun trading bot with complete buy/sell functionality
    """
    ...
    def __init__(self, config: TradeConfig = None):
        ...
        # Instruction discriminators (discovered working values)
        self.BUY_DISCRIMINATOR = bytes.fromhex("66063d1201daebea")
        self.SELL_DISCRIMINATOR = bytes.fromhex("33e685a4017f83ad")
        
        # Token-specific addresses (for working test token)
        self.WORKING_TOKEN_MINT = Pubkey.from_string("6nTfw8wkRRqJviT9JZvRyeR2WV682ETZEf7LudQzpump")
        self.WORKING_BONDING_CURVE = Pubkey.from_string("9y2pMaFYhLasS5guL1YyfBSfXzqybMyxcCn5XcawRjfb")
        self.WORKING_BONDING_CURVE_ATA = Pubkey.from_string("HfQEZpR8wnKk3qTiUg1EjhAtzGcHdLKzGsavJUQLEFcz")
        
        logger.info(f"🤖 PumpFun Trading Bot initialized")
    ...
    def _build_trade_accounts(self, token_mint: Pubkey, bonding_curve: Pubkey, bonding_curve_ata: Pubkey, creator_vault: Pubkey = None) -> list[AccountMeta]:
        """Build the account list for pump.fun trade instructions"""
        our_token_account = get_associated_token_address(self.wallet_pubkey, token_mint)
        
        # Use provided creator vault or fall back to default
        vault_to_use = creator_vault or self.CREATOR_VAULT
        
        # Accounts order is critical and must match the Anchor program's account validation
        return [
            AccountMeta(self.GLOBAL_ACCOUNT, is_signer=False, is_writable=True),  # Global state account (must be first and owned by PUMP_PROGRAM)
            AccountMeta(self.FEE_RECIPIENT, is_signer=False, is_writable=True),  # Fee recipient
            AccountMeta(token_mint, is_signer=False, is_writable=True),  # Token mint (must be SPL token)
    ...
    async def execute_buy_trade(
        ...
        try:
            ...
            bonding_curve_info = await self.client.get_account_info(bonding_curve)
            if not bonding_curve_info.value:
                raise Exception(f"Bonding curve {bonding_curve} not found")

            # Verify bonding curve ATA exists
            curve_ata_info = await self.client.get_account_info(bonding_curve_ata)
            if not curve_ata_info.value:
                raise Exception(f"Bonding curve ATA {bonding_curve_ata} not found")
            ...
            # Build trade accounts with the optimal creator vault
            accounts = self._build_buy_accounts(token_mint, bonding_curve, bonding_curve_ata, creator_vault)
    ...
    async def execute_sell_trade(
        ...
        try:
            ...
            # Build trade accounts with the optimal creator vault
            accounts = self._build_sell_accounts(token_mint, bonding_curve, bonding_curve_ata, creator_vault)
